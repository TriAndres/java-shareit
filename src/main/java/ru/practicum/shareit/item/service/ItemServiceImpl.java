...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemDto create(ItemDto itemDto, long userId) {
        ...
        if (itemDto.getAvailable() == null || itemDto.getDescription() == null || itemDto.getName() == null) {
            throw new RuntimeException("Null fields in ItemDto element!");
        }
        if (itemDto.getName().isEmpty() || itemDto.getDescription().isEmpty()) {
            throw new RuntimeException("Empty fields in ItemDto element!");
        }
    ...
    private void checkingUserId(long userId) {
        if (userId == -1) {
            throw new RuntimeException("There is no user with header-Id : " + userId);
        }
